...
------------------------------------------------------------------FIND A WAY TO ADD CONSTRAINTS FOR COMPOSITE KEYS
...
declare
    tmpval number;
BEGIN
    select SQ_CUSTOMER_ID.NEXTVAL into tmpval from DUAL;
    :NEW.CUST_ID := tmpval;
END;
/

create or replace trigger TR_ACCOUNT_ID
before insert on CUST_ACCOUNT
for each row
declare
    tmpval number;
BEGIN
    select SQ_ACCOUNT_ID.NEXTVAL into tmpval from DUAL;
    :NEW.ACC_ID := tmpval;
END;
/

create or replace trigger TR_TRANSACTION_ID
before insert on ACC_TRANSACTION
for each row
declare
    tmpval number;
BEGIN
    select SQ_TRANSACTION_ID.NEXTVAL into tmpval from DUAL;
    :NEW.TRANS_ID := tmpval;
END;
/
/*******************************************************************************
